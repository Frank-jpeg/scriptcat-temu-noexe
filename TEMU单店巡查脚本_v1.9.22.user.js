...
(function () {
  ...
  const DEFAULT_CONFIG = {
    ...
    loginMethod: 'phone',
    lowDeclaredPriceThreshold: LOW_DECLARED_PRICE_THRESHOLD,
    protectDiff: true,
    protectDiffLimit: 1,
  ...
  async function loadConfig() {
    ...
    merged.loginMethod = merged.loginMethod === 'email' ? 'email' : 'phone';
    merged.lowDeclaredPriceThreshold = Math.max(0, toFloat(merged.lowDeclaredPriceThreshold, LOW_DECLARED_PRICE_THRESHOLD));
  ...
  function createJob(config) {
    ...
    return {
      ...
      lowDeclaredPriceThreshold: Math.max(0, toFloat(config.lowDeclaredPriceThreshold, LOW_DECLARED_PRICE_THRESHOLD)),
      results: {},
      logs: [],
      error: '',
  ...
  function buildManualReasons(results, enabledChecks) {
    ...
    if (enabledChecks.urgent_declared_price && toInt(results.urgent_declared_price && results.urgent_declared_price.lowPriceCount) > 0) {
      const threshold = Math.max(0, toFloat(results.urgent_declared_price.lowPriceThreshold, LOW_DECLARED_PRICE_THRESHOLD));
      reasons.push(`待发货低申报价${toInt(results.urgent_declared_price.lowPriceCount)}条（阈值¥${threshold.toFixed(2)}）`);
    }
  ...
  function truncateDiagnosticText(value, maxLength = 120) {
  ...
  function buildUrgentLowPriceDetails(result) {
    const rows = Array.isArray(result && result.rows) ? result.rows : [];
    return rows.slice(0, 20).map((row, index) => {
      const orderMatch = normalize(row.prepareOrderNo).match(/\bWB\d{6,}\b/);
      const orderNo = orderMatch ? orderMatch[0] : normalize(row.prepareOrderNo) || `第${index + 1}条`;
      const declaredPriceText = normalize(row.declaredPriceText) || `¥${toFloat(row.declaredPrice).toFixed(2)}`;
      const status = normalize(row.status) || '-';
      const createdTime = normalize(row.createdTime) || '-';
      const product = truncateDiagnosticText(row.productInfo || row.skuInfo || '-', 140);
      const sourceRowPreview = truncateDiagnosticText(row.sourceRowPreview || '-', 240);
      return {
        orderNo,
        declaredPriceText,
        status,
        ...
        product,
        sourceRowPreview,
        summary: `${orderNo}｜申报价 ${declaredPriceText}｜状态 ${status}｜创建 ${createdTime}｜商品 ${product}`,
      };
    });
  }
  ...
  function buildCheckSummaries(results, enabledChecks) {
    ...
    if (enabledChecks.urgent_declared_price && results.urgent_declared_price) {
      ...
      const threshold = Math.max(0, toFloat(results.urgent_declared_price.lowPriceThreshold, LOW_DECLARED_PRICE_THRESHOLD));
      const details = buildUrgentLowPriceDetails(results.urgent_declared_price);
      items.push({
        key: 'urgent_declared_price',
        label: '待发货低申报价',
        manual: lowPriceCount > 0,
        text: `低于 ¥${threshold.toFixed(2)}：${lowPriceCount} 条`,
        details,
      });
    }
    ...
    if (checkSummaries.length) {
      ...
      checkSummaries.forEach((item) => {
        ...
        if (Array.isArray(item.details) && item.details.length) {
          item.details.forEach((detail) => lines.push(`  - ${detail.summary}`));
        }
      });
    }
    if (job.error) {
              ...
              ${Array.isArray(item.details) && item.details.length ? `
                <div style="margin-top:8px;display:flex;flex-direction:column;gap:6px;">
                  ${item.details.map((detail) => `
                    <div style="padding:7px 8px;border:1px solid #4b3a29;background:#17120e;border-radius:6px;color:#e5e7eb;">
                      <div style="font-weight:800;color:#fde68a;">${escapeHtml(detail.orderNo)} · ${escapeHtml(detail.declaredPriceText)}</div>
                      <div style="margin-top:2px;">状态：${escapeHtml(detail.status)}　创建：${escapeHtml(detail.createdTime)}</div>
                      <div style="margin-top:2px;word-break:break-all;">商品：${escapeHtml(detail.product)}</div>
                      <details style="margin-top:4px;color:#94a3b8;">
                        <summary style="cursor:pointer;">查看脚本读取的原始行</summary>
                        <div style="margin-top:3px;white-space:pre-wrap;word-break:break-all;">${escapeHtml(detail.sourceRowPreview)}</div>
                      </details>
                    </div>
                  `).join('')}
                </div>
  ...
  function findLoginMethodTab(loginMethod) {
    const label = loginMethod === 'email' ? '邮箱登录' : '手机号登录';
    const classHint = loginMethod === 'email' ? 'email' : 'phone';
    const candidates = Array.from(document.querySelectorAll('button, [role="tab"], [role="button"], a, div, span'))
      .filter((element) => visible(element) && normalize(element.innerText) === label);
    candidates.sort((left, right) => {
      const score = (element) => {
        let value = 0;
        ...
        const className = String(element.className || '').toLowerCase();
        if (className.includes('tab') || className.includes('login') || className.includes(classHint)) {
          value += 10;
        }
  ...
  function isSiteMainPageUrl(href = location.href) {
    let isSiteMainPath = String(href || '').includes('/settle/site-main');
    try {
      isSiteMainPath = new URL(href, location.origin).pathname === '/settle/site-main';
    } catch (_error) {}
    return isSiteMainPath;
  }

  function isSiteMainAuthorizationPage(bodyText = '') {
    const body = normalize(bodyText || document.body.innerText || '');
    return isSiteMainPageUrl()
      && body.includes('Seller Central')
      && body.includes('账号ID')
      && body.includes('店铺名称')
      && body.includes('隐私政策')
      && (body.includes('共享') || body.includes('授权'));
  }

  function findSiteMainAuthorizationButton(bodyText = '') {
    if (!isSiteMainAuthorizationPage(bodyText)) {
      return null;
    }
    const candidates = Array.from(document.querySelectorAll('button, [role="button"], a'))
      .filter((element) => visible(element)
        && !isDisabled(element)
        && normalize(element.innerText || element.value || '') === '进入');
    candidates.sort((left, right) => {
      const score = (element) => {
        let value = element.tagName === 'BUTTON' ? 30 : 10;
        let current = element;
        const contextTexts = [];
        for (let level = 0; current && level < 4; level += 1, current = current.parentElement) {
          contextTexts.push(current.innerText || '');
        }
        const context = normalize(contextTexts.join(' '));
        if (context.includes('全球')) value += 40;
        if (context.includes('商家中心')) value += 15;
        const className = String(element.className || '').toLowerCase();
        if (className.includes('primary') || className.includes('enter')) value += 10;
        return value;
      };
      return score(right) - score(left);
    });
  ...
  function findAccessAuthorizationButton(bodyText = '') {
    const standardButton = Array.from(document.querySelectorAll('button, [role="button"], a'))
    ...
    return standardButton || findSiteMainAuthorizationButton(bodyText);
  }

  function buildLoginStopMessage(state) {
    if (state && state.loginPrompt && isLoginPageUrl()) {
  ...
  async function tryLoginFromPage(jobId, loginState) {
    ...
    const loginMethod = config.loginMethod === 'email' ? 'email' : 'phone';
    const loginMethodLabel = loginMethod === 'email' ? '邮箱登录' : '手机号登录';
    const loginMethodTab = findLoginMethodTab(loginMethod);
    if (loginMethodTab) {
      await updateJobMessage(`检测到登录页，正在切换到${loginMethodLabel}...`);
      await appendJobLog('WARN', `按本浏览器设置切换到${loginMethodLabel}`);
      loginMethodTab.click();
    ...
    job.currentMessage = `检测到登录页，正在使用${loginMethodLabel}自动勾选协议并登录...`;
    await saveJob(job);
    await appendJobLog('WARN', `检测到登录页，使用${loginMethodLabel}勾选协议并点击登录`);
  ...
  function normalizeInspectionTargetUrl(value) {
    if (!value) {
      return '';
    }
    try {
      const candidate = new URL(value, location.origin);
      if (candidate.protocol !== 'https:' || isLoginPageUrl(candidate.href) || isSiteMainPageUrl(candidate.href)) {
        return '';
      }
      const matchesKnownTarget = Object.values(URLS).some((targetValue) => {
        const target = new URL(targetValue);
        return candidate.origin === target.origin && candidate.pathname === target.pathname;
      });
      return matchesKnownTarget ? candidate.href : '';
    } catch (_error) {
      return '';
    }
  }

  async function bypassSiteMainPage(jobId) {
    if (!isSiteMainPageUrl()) {
      return false;
    }
    const currentJob = await loadJob();
    if (!currentJob || currentJob.id !== jobId || currentJob.status !== 'running') {
      return false;
    }
    let redirectTarget = '';
    try {
      const currentUrl = new URL(location.href);
      redirectTarget = normalizeInspectionTargetUrl(currentUrl.searchParams.get('redirectUrl'));
    } catch (_error) {}
    const stepKey = Array.isArray(currentJob.steps) ? currentJob.steps[currentJob.stepIndex] : '';
    const stepTarget = normalizeInspectionTargetUrl(URLS[stepKey]);
    const targetUrl = redirectTarget || stepTarget;
    if (!targetUrl) {
      await updateJobMessage('检测到商家中心中转页，但未找到安全的巡查目标地址，正在等待...');
      await checkedSleep(jobId, 500, 100);
      return true;
    }
    const targetLabel = stepKey ? getStepLabel(stepKey) : '当前巡查目标页';
    await updateJobMessage(`检测到商家中心中转页，正在直接前往${targetLabel}...`);
    await appendJobLog('WARN', `跳过商家中心中转页，直接打开${targetLabel}`);
  ...
  function readAccessState() {
    ...
    const authButton = findAccessAuthorizationButton(body);
    ...
    const authorizationPrompt = (body.includes('即将前往')
      ...
      || isSiteMainAuthorizationPage(body);
    return {
      body,
      hasRegionPage,
      regionButtons,
  ...
  async function ensureAccessReady(jobId) {
    if (await bypassSiteMainPage(jobId)) {
      return false;
    }
    ...
    const hasVisibleLoginMethodTab = !!(findLoginMethodTab('phone') || findLoginMethodTab('email'));
    if (isLoginPageUrl() && hasVisibleLoginMethodTab) {
    ...
    if (state.authorizationPrompt || state.authButton) {
      ...
      const authButton = findAccessAuthorizationButton(state.body) || state.authButton;
      if (authButton && !isDisabled(authButton)) {
        clickLikeUser(authButton);
        await checkedSleep(jobId, 2500);
        return false;
      }
      await updateJobMessage('授权协议已勾选，正在等待授权确认按钮可用...');
      await checkedSleep(jobId, 500, 100);
      return false;
    }
    if (isLoginPageUrl()) {
  ...
  function extractUrgentPendingPriceRows(threshold = LOW_DECLARED_PRICE_THRESHOLD) {
    const lowPriceThreshold = Math.max(0, toFloat(threshold, LOW_DECLARED_PRICE_THRESHOLD));
    const body = normalize(document.body.innerText || '');
    const headers = Array.from(document.querySelectorAll('thead th')).map((cell) => normalize(cell.innerText || ''));
    ...
    const parsedRows = [];
    let validPendingRowCount = 0;
    for (const item of rows) {
      const mapped = item.mapped || {};
      const cells = item.cells || [];
      const prepareOrderText = mapped['备货单号'] || cells[0] || '';
      const prepareOrderMatch = normalize(prepareOrderText).match(/\bWB\d{6,}\b/);
      const status = normalize(mapped['状态'] || cells[2] || '');
      if (!prepareOrderMatch || !status.includes('待发货')) {
        continue;
      }
      validPendingRowCount += 1;
      ...
      if (declaredPrice < 0 || declaredPrice >= lowPriceThreshold) {
        continue;
      }
      parsedRows.push({
        prepareOrderNo: prepareOrderMatch[0],
        ...
        remark: `申报价格低于${lowPriceThreshold}`,
        isLowPrice: true,
        sourceRowPreview: truncateDiagnosticText(cells.join(' | '), 1200),
      });
    }
    return {
      totalRowCount: validPendingRowCount,
      totalVisibleRowCount: rows.length,
      headers,
      hasNoData: body.includes('暂无数据'),
      rows: parsedRows,
  ...
  async function waitExtractUrgentPendingPriceRows(jobId, threshold) {
    const deadline = Date.now() + 25000;
    while (Date.now() < deadline) {
      await assertNotStopped(jobId);
      const result = extractUrgentPendingPriceRows(threshold);
      if ((result.totalRowCount && result.totalRowCount > 0) || result.hasNoData || /暂无数据/.test(result.bodyPreview || '')) {
        return result;
      }
  ...
  async function stepUrgentDeclaredPrice(job) {
    ...
    while (Date.now() < deadline) {
      ...
      const result = await waitExtractUrgentPendingPriceRows(job.id, job.lowDeclaredPriceThreshold);
      return { completed: true, data: result };
    }
    throw new Error('等待检查待发货低申报价页超时');
  }
  ...
  async function readConfigFromUi() {
    ...
    return {
      ...
      loginMethod: panel.querySelector('[data-role="login-method"]').value === 'email' ? 'email' : 'phone',
      lowDeclaredPriceThreshold: Math.max(0, toFloat(panel.querySelector('[data-role="low-declared-price-threshold"]').value, LOW_DECLARED_PRICE_THRESHOLD)),
  ...
  async function renderPanel() {
    ...
    panel.querySelector('[data-role="login-method"]').value = config.loginMethod === 'email' ? 'email' : 'phone';
    ...
    const lowDeclaredPriceThresholdInput = panel.querySelector('[data-role="low-declared-price-threshold"]');
    const ruleTextInput = panel.querySelector('[data-role="rule-text"]');
    ...
    if (document.activeElement !== lowDeclaredPriceThresholdInput) {
      lowDeclaredPriceThresholdInput.value = String(config.lowDeclaredPriceThreshold ?? LOW_DECLARED_PRICE_THRESHOLD);
    }
  ...
  function buildPanelDom() {
    ...
    panel.innerHTML = `
      ...
      <div id="${APP_ID}_body" style="padding:10px;">
        ...
        <div style="margin-bottom:8px;padding:7px 9px;border:1px solid #25303a;border-radius:8px;background:#161b22;display:flex;align-items:center;gap:7px;flex-wrap:wrap;">
          <span style="font-weight:600;">本浏览器登录方式</span>
          <select data-role="login-method" style="${inputStyle('104px')}">
            <option value="phone">手机号登录</option>
            <option value="email">邮箱登录</option>
          </select>
          <span style="font-size:10px;color:#94a3b8;">每个店铺浏览器设置一次</span>
        </div>
        ...
        <div style="margin-bottom:8px;padding:7px;border:1px solid #25303a;border-radius:8px;background:#161b22;">
          ...
          <label style="display:flex;align-items:center;gap:6px;margin-top:7px;flex-wrap:wrap;">
            <span>低申报价阈值</span>
            <span>¥</span>
            <input type="number" data-role="low-declared-price-threshold" min="0" step="0.1" style="${inputStyle('72px')}">
            <span style="font-size:10px;color:#94a3b8;">申报价低于此值时提醒</span>
          </label>
    ...
    panel.querySelector('[data-role="login-method"]').addEventListener('change', async () => {
      const nextConfig = await readConfigFromUi();
      await saveConfig(nextConfig);
      scheduleRender();
    });

    panel.querySelectorAll('input[data-check], [data-role="protect-diff"], [data-role="protect-limit"], [data-role="low-declared-price-threshold"]').forEach((input) => {
